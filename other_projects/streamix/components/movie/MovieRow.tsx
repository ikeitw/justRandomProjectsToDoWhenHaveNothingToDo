...
export default function MovieRow({ title, movies, size = 'md', loading = false }: MovieRowProps) {
    ...
    const scroll = (dir: 'left' | 'right') => {
        ...
        rowRef.current.scrollBy({ left: dir === 'right' ? rowRef.current.clientWidth * 0.75 : -rowRef.current.clientWidth * 0.75, behavior: 'smooth' });
    };

    if (loading) {
        // Show placeholder skeletons while loading movies
        return (
            <div className="mb-8">
