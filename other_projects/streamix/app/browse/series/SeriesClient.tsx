...
const GENRES = [
  'Action & Adventure', 'Comedy', 'Drama', 'Sci-Fi & Fantasy', 'Mystery',
  'Crime', 'Animation', 'Kids', 'Documentary', 'Reality', 'Thriller',
  'Horror', 'Romance', 'Western', 'History', 'War & Politics',
];
...
export default function SeriesClient() {
  ...
  const fetchMore = useCallback(async (page: number, reset: boolean) => {
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ mediaType: 'series', page: String(page) });
      if (q) params.set('q', q);
  ...
  const setParam = (key: string, value: string) => {
    ...
    router.replace(`/browse/series?${p}`, { scroll: false });
  };

  const clearFilters = () => router.replace('/browse/series', { scroll: false });
      ...
      <div className="px-4 sm:px-8 max-w-[1800px] mx-auto pb-16">
        ...
        {!loading && displayed.length === 0 && allItems.length > 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-[var(--dim)] text-sm mb-4">Try adjusting your filters</p>
        ...
        {!loading && allItems.length === 0 && (
          <div className="text-center py-24">
            <p className="text-5xl mb-4">📺</p>
            <p className="text-white font-semibold text-lg mb-1">No series found</p>
