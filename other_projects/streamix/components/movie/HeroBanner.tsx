...
export default function HeroBanner({ movies }: { movies: Movie[] }) {
  // Control fade transitions between featured movie slides
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isTransitioning, setIsTransitioning] = useState(false);

  // First 5 movies for rotation
      ...
      <div className="relative w-full h-[52vw] max-h-[650px] min-h-[380px] overflow-hidden">
        {backdropUrl && (
            ...
            <img src={backdropUrl} alt={movie.title}
                 className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-400 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`} />
        )}

        {/* Multi-layered gradient for text readability */}
        <div className="hero-gradient absolute inset-0" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#03171E]/20 to-transparent mix-blend-multiply" />
        ...
        <div className={`absolute inset-0 flex flex-col justify-end pb-16 px-8 sm:px-14 transition-opacity duration-400 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
          <div className="max-w-xl">
            ...
            <span className="text-[#00A0EC] text-xs font-bold uppercase tracking-widest">
              ...
              {year && <span className="text-[#7a9caa] text-xs">{year}</span>}
            </div>
            ...
            <span className="flex items-center gap-1 text-yellow-400 font-semibold text-sm">
              ...
              <span className={`text-xs font-bold px-2 py-0.5 rounded ${movie.vote_average > 7 ? 'bg-green-900/60 text-green-400 border border-green-700/40' : 'bg-yellow-900/60 text-yellow-400 border border-yellow-700/40'}`}>
            ...
            <p className="text-[#b3cdd6] text-sm leading-relaxed mb-5 line-clamp-2 max-w-lg">
              {movie.overview}
            </p>
        ...
        <div className="absolute bottom-6 right-8 flex items-center gap-1.5">
          {featured.map((_, i) => (
              <button key={i} onClick={() => { setCurrentIndex(i); setIsTransitioning(false); }}
